...
THITAp
% plot(Xp,Yp,'--or');
% title('Modelo Cinematico: Grandes dV')
% xlabel('X[m]')
% ylabel('Y[m]')
...
nL=[2,12,25,22,23,17,35,61,55,102];
nR=[3,3,7,43,9,6,4,70,49,115];

% nR=[4,2,7,41,8,5,1,67,46,109];
% nL=[0,10,25,20,23,17,32,59,52,95];
...
% figure
% plot(Xp_m,Yp_m,'-ob');
% title('Mediciones: Muchas variaciones de velocidad')
% xlabel('X[m]')
% ylabel('Y[m]')

figure('Name','Modelo Cinematico: Grandes Variaciones')
hold on
title('Modelo Cinematico: Grandes Variaciones')
plot(Xp,Yp,'--or');
plot(Xp_m,Yp_m,'-ob');
...
figure('Name','Errores en Gran Variacion')
hold on
title('Errores en Gran Variacion')
plot(err_vL,'--or');
plot(err_vR,'-ob');
xlabel('n')
ylabel('error[m/s]')
